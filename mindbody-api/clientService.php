...
class MBClientService extends MBAPIService 
{
	...
	{	
        ...
        public function GetClientSchedule($clientId, $PageSize = null, $CurrentPage = null, $XMLDetail = XMLDetail::Full, $Fields = null, SourceCredentials $credentials = null) {
		$additions['ClientID'] = $clientId;
		$params = $this->GetMindbodyParams($additions, $this->GetCredentials($credentials), $XMLDetail, $PageSize, $CurrentPage, $Fields);
		
		try
		{
			$result = $this->client->GetClientSchedule($params);
		}
		catch (SoapFault $fault)
		{
			DebugResponse($result);
